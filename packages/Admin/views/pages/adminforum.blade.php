...
<script>
    ...
    var dateFormater = [];
    app.controller('ForumController',function(URL,paginationServices,$scope,$http,$filter,$location){
...
////                dateFormater = response.data.response[0].created_at.split(' ');
////                dateFormater[0];
////                console.log(dateFormater[0]);
////                $scope.date = dateFormater[0];
//               
//            });
//
...
//
    ...
    $scope.getTopicsList=function(offset,limit,sort,sort_order,search,status) {
        ...
        then(function(response) {
			...
			var currentPage = 1;
			if (pageStatus === '0' && searchStatus === '0'){
				if (pageOffset !== '0' ){
					currentPage = (pageOffset/limit) + 1;
				}else{
					currentPage = 1;
				}
    ...
    $scope.$on('pagination:updated', function(event,data) {
		...
		// Back Button logic :
		if (pageStatus === '0'){
			$scope.pagination.offset = pageOffset !== '0' ? pageOffset : 0;
		}
		if (searchStatus === '0'){
			if (search !== ""){
				$scope.search = search;
...
<div ng-controller="ForumController" class="col-lg-12">
    <div class="row">
        <div class="col-lg-12 form-group">
            <input ng-model='search' ng-change='searchtopics()' class="form-control pull-left" placeholder="Search By Topic" style="width: 200px;">
            <span class="pull-right">
                <button type="button" class="btn btn-primary" ng-click="openForum()">Start Topic</button>
            </span>
        </div>
    </div>
    <div class="row">
            ...
            <table class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                    ...
                    <tr ng-if="pagination.total > 0" ng-repeat="topic in topics">
                        ...
                        <td><a href="{{route('admin.forumreply','')}}/@{{topic.id}}/@{{pagination.offset}}/@{{search}}">@{{topic.title}}</a></td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" ng-click="closeForum()" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Start Topic</h4>
                </div>
                <div class="modal-body">
                    <form ng-submit="submit()" id="forum-form"
						...
						<button type="submit" class="btn btn-primary">Create Topic</button>
                        <button class="btn btn-primary" type="button" ng-click="closeForum()" >Cancel</button>
					</form>
               <link href="{!! asset('css/loader.css') !!}" rel="stylesheet" type="text/css" />
            <div id="loader" class="loading">Loading&#8230;</div>
          </div>
